...
import { sortByKey, getActionState, setActionState } from "../views/AngularWorkflow.jsx";
...
const ParsedAction = (props) => {
	...
	const [activeMainTab, setActiveMainTab] = useState("setup")
	...
	*/
			...
			if (environments !== undefined && environments !== null && environments.length > 0) {
				if (selectedAction.environment !== undefined && selectedAction.environment !== null) {
					...
					if (foundenv !== undefined && foundenv !== null) {
						...
						// Fallback: ensure we always have some environment selected
						setSelectedActionEnvironment(environments[0])
					}
				} else {
					// No environment set on the action: default to first available
					setSelectedActionEnvironment(environments[0])
				}
			}
		}
	...
	}, [selectedActionEnvironment, selectedAction, environments])
	...
	const getHttpMethodFromName = (actionName) => {
		if (actionName === undefined || actionName === null) {
			return ""
		}

		const lowerName = actionName.toLowerCase()
		if (lowerName.includes("get_")) {
			return "GET"
		} else if (lowerName.includes("post_")) {
			return "POST"
		} else if (lowerName.includes("put_")) {
			return "PUT"
		} else if (lowerName.includes("patch_")) {
			return "PATCH"
		} else if (lowerName.includes("delete_")) {
			return "DELETE"
		} else if (lowerName.includes("options_")) {
			return "OPTIONS"
		} else if (lowerName.includes("connect_")) {
			return "CONNECT"
		}
	...
	const ActionSelectOption = (actionprops) => {
		...
		return (
				...
				componentsProps={{
					tooltip: {
						sx: {
							...
							border: `1px solid ${theme.palette.slateGrayColor || "rgba(255,255,255,0.15)"}`,
							borderRadius: 4,
							padding: "14px 16px",
							maxWidth: 320,
							fontSize: 14,
							...
							"& .MuiTooltip-arrow": {
								color: theme.palette.surfaceColor,
								"&::before": {
									border: `1px solid ${theme.palette.slateGrayColor || "rgba(255,255,255,0.15)"}`,
								}
							}
			...
			>
				...
				>
					<div style={{ display: "flex", alignItems: "center", marginBottom: extraDescription.length > 0 ? 4 : 0 }}>
						<span style={{ marginRight: 12, display: "flex", alignItems: "center", justifyContent: "center", width: 24, height: 24, flexShrink: 0 }}>
							{useIcon}
						</span>
						<div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", justifyContent: "center" }}>
							<div style={{ fontWeight: 500, fontSize: 14, color: theme.palette.textPrimary, lineHeight: 1.4 }}>{newActionname}</div>
							{extraDescription.length > 0 ?
								<Typography variant="body2" style={{ marginTop: 2, overflow: "hidden", whiteSpace: "nowrap", textOverflow: "ellipsis", display: "block", color: theme.palette.text.secondary, fontSize: 12, lineHeight: 1.4 }}>
									{extraDescription}
								</Typography>
								: null}
						</div>
	...
	if (newAppname === undefined || newAppname === null) {
		...
		try {
			if (typeof newAppname === "string") {
				newAppname = newAppname.replace(/_/g, " ")
			}
		} catch (e) {
		...
		<div style={appApiViewStyle} id="parsed_action_view">
			{isSidebarLoading ? (
				<div style={{ paddingTop: 24 }}>
					...
					{/* A couple of label + field rows to mimic config inputs */}
					{[1, 2, 3].map((row) => (
						<div key={row} style={{ marginBottom: 18 }}>
							...
							<Skeleton
								...
								style={{
									width: "100%",
									borderRadius: theme.palette?.borderRadius || 8,
									backgroundColor:
										themeMode === "dark"
			...
			<Tabs
				...
				onChange={(event, newValue) => {
					...
					if (selectedAction?.id) {
						setActionState(selectedAction.id, { tab: newValue, userSelectedTab: true }, workflow?.id)
					}
				}}
				variant="fullWidth"
				...
				style={{
					marginBottom: 10,
					minHeight: 50,
					borderBottom: `1px solid ${theme.palette.slateGrayColor}`,
				}}
			>
				<Tab
					label="Setup"
					...
					style={{
						...
						fontWeight: activeMainTab === "setup" ? 600 : 400,
						alignItems: "center",
						fontFamily: theme.typography.fontFamily,
				...
				<Tab
					...
					style={{
						...
						fontFamily: theme.typography.fontFamily,
						color: theme.palette.text.primary,
						fontWeight: activeMainTab === "configuration" ? 600 : 400,
						alignItems: "center",
						justifyContent: "center",
					}}
				/>
			...
			{(activeMainTab === "setup" || activeMainTab === "configuration") ? (
			<React.Fragment>
			<div style={{ position: "relative" }}>
			{hideExtraTypes === true ? null : (
				<span>
					<div style={{ display: "flex", minHeight: 40, marginBottom: 10 }}>
						<div style={{ flex: 1 }}>
								...
								<Tooltip title={"App: " + selectedAction.app_name + ". Click to open in new tab"} placement="top">
									<a href={"/apps/" + selectedAction?.app_id} target="_blank" style={{ textDecoration: "none", color: theme.palette.textPrimary, }}>
										<img src={selectedAppIcon} style={{
											...
											borderRadius: 8,
											marginTop: 13,
											border: themeMode === "dark" ? "1px solid rgba(255,255,255,0.3)" : "1px solid rgba(0, 0, 0, 0.1)",
										}} />
									</a>
								</Tooltip>
							...
							{activeMainTab === "setup" && (
							<div style={{ display: "flex", marginTop: 0, paddingTop: 30 }}>
								<IconButton
									style={{
										marginTop: "auto",
										marginBottom: "auto",
					...
					<div style={{ display: "flex", gap: 5 }}>
						<div style={{ flex: 5 }}>
							...
							<TextField
								style={theme.palette.actionSidebarField}
								InputProps={{
									disableUnderline: true,
									style: {
										backgroundColor: theme.palette.actionSidebarField.backgroundColor,
										color: theme.palette.actionSidebarField.color,
										borderRadius: theme.palette.actionSidebarField.borderRadius,
										height: 40,
										fontFamily: theme.typography.fontFamily,
									},
									sx: {
			...
			{false && selectedApp.name !== undefined &&
...
{/* Change made in new release when we added Tabs system in it */}
					...
					>
						<ExpandMoreIcon
							style={{
								...
								fontSize: 20,
								color: theme.palette.textPrimary,
								transform: authSectionOpen ? "rotate(0deg)" : "rotate(-90deg)",
						...
						<div style={{ display: "flex", }}>
							<Select
								...
								style={{
									backgroundColor: theme.palette.actionSidebarField.backgroundColor,
									color: theme.palette.actionSidebarField.color,
									height: theme.palette.actionSidebarField.height,
		...
		{/* :) will be changed soon : Find action */}
			<div style={{
				position: "absolute",
				top: 16.5,
				left: 55,
				...
				{isAgent ? (
					...
					<Autocomplete
						...
						renderOption={(props, option, state) => {
							...
							const iconColor = theme.palette?.mode === "dark" ? "white" : "#333";
							const useIcon = iconInfo.originalIcon && iconInfo.originalIcon !== "" 
								? <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 22, height: 22, color: iconColor, fontSize: 22 }}>{iconInfo.originalIcon}</span>
								: null;
							...
							var method = getHttpMethodFromName(option.name)
						...
						renderInput={(params) => {
							...
							const currentMethod = getHttpMethodFromName(selectedAction !== undefined && selectedAction !== null ? selectedAction.name : "")
							...
							return (
								...
								>
									<TextField
										...
										InputProps={{
											...params.InputProps,
											startAdornment: currentMethod.length > 0 ? (
												<span style={{ color: "#2ecc71", fontWeight: 600, marginRight: 8, fontSize: 13 }}>
													{currentMethod}
												</span>
											) : params.InputProps?.startAdornment,
										}}
										placeholder={isIntegration ? "Choose a category" : "Find Actions"}
										variant="outlined"
			...
			<Button
				...
				fullWidth
				>
					<SettingsIcon style={{ marginRight: 10, fontSize: 20 }} />
					Go to configuration
			</Button>
			)}
			</div>
					...
					*/}
								...
								if (data.name !== "queries" && data.name !== "key" && data.name !== "value" ) {
									...
									var hideBodyButtonValue = (
										...
										>
											<ToggleButtonGroup
												value={hideBody === true ? "advanced" : "simple"}
												exclusive
												onChange={(_, val) => {
													if (val === "simple") {
														// Set localstorage
											...
											>
												<ToggleButton value="simple" sx={toggleButtonSx}>
													Simple
												</ToggleButton>
												<ToggleButton value="advanced" sx={toggleButtonSx}>
													Advanced
												</ToggleButton>
											</ToggleButtonGroup>
											{/* Legacy Tabs kept for reference, but logic is now handled by ToggleButtonGroup */}
											{/*<Tabs
									...
									var datafield = (
										...
										>
											<TextField
												...
												InputProps={{
													style: {
														maxHeight: multiline === true ? undefined : theme.palette.actionSidebarField.height,
														backgroundColor: theme.palette.actionSidebarField.backgroundColor,
														borderRadius: theme.palette.actionSidebarField.borderRadius,
														width: "100%",
														minHeight: theme.palette.actionSidebarField.height,
														color: theme.palette.actionSidebarField.color,
													},
													sx: {
														'& .MuiOutlinedInput-notchedOutline': {
															...
															<div style={{ display: "flex", gap: 8 }}>
																<TextField
																	...
																	style={{ 
																		...
																		borderRadius: theme.palette.actionSidebarField.borderRadius,
																		height: theme.palette.actionSidebarField.height,
																		border: theme.palette.actionSidebarField.border,
																	}}
																	InputProps={{
																		style: {
																			backgroundColor: theme.palette.actionSidebarField.backgroundColor,
									...
									return (
										<div key={data.name} style={{ marginTop: isFirstOptional ? 55 : 5, }}>
											{isFirstOptional ? (
												<>
													<Divider style={{ backgroundColor: "rgba(255,255,255,0.1)", marginBottom: 10, }} />
													<Button
														fullWidth
														onClick={() => {
															if (selectedAction?.id) {
																const newValue = !optionalParamsOpen
																setOptionalParamsOpen(newValue)
																setActionState(selectedAction.id, { showOptionalParams: newValue }, workflow?.id)
															}
														}}
														style={{
															textTransform: "none",
														...
														endIcon={<ExpandMoreIcon style={{ transform: optionalParamsOpen ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 0.3s' }} />}
													>
														Optional Parameters
													</Button>
												</>
											) : null}
											<Collapse in={optionalParamsOpen || !isOptional}>
											<div>
