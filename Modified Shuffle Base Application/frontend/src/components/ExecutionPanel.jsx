...
import { CheckCircle, Error, ArrowBack, Close, Cached as CachedIcon, Pause as PauseIcon, Hub as HubIcon } from '@mui/icons-material';
...
const ExecutionDetail = ({ execution: initialExecution, onBack, globalUrl, onExecutionUpdate, selectedAction, executeWorkflow }) => {
  ...
  const handleReverseWorkflow = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${globalUrl}/api/v1/reverse-workflow`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      console.log("Reverse Workflow Result:", data);

      toast.success("Reverse workflow analysis started");

    } catch (error) {

      console.error(error);

      toast.error("Reverse workflow failed");
  ...
  return (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <Tooltip title="Reverse workflow analysis" placement="top">
        <IconButton
          size="small"
          onClick={handleReverseWorkflow}
