...
def _build_reverse_system_prompt(
    ...
    for i, action in enumerate(graph_records):
        ...
        graph_lines.append(
            f"  [{i+1}] name={action.get('name')}"
            f" | app={action.get('app_name')} v{action.get('app_version')}"
            f" | id={action.get('id')}"
            f" | large_image={action.get('large_image') or ''}"
...
async def generate_reverse_from_graph(
    ...
    logger.info("GRAPH SIZE = %d", len(compressed_graph))
    logger.info("APP CATALOG SIZE = %d", len(app_catalog.get("apps", [])))
    
    # 3. GRAPH CONTEXT (HARUS DI SINI)
    graph_context: List[Dict[str, Any]] = []  # default aman: jika query gagal, degrade gracefully (bukan crash)
    try:
    ...
    # 4b. RAG EXAMPLES — playbook serupa, dicari dari apps + action names yang
    #     sebenarnya dipakai di workflow ini (bukan dari deskripsi NL, karena
    #     reverse pipeline tidak punya deskripsi NL seperti forward pipeline).
    rag_query = " ".join(
    ...
    if rag_query:
        try:
            rag_hits = await recommender.recommend(rag_query, top_k=2)
            rag_examples = [
                f"{hit.get('name', '')}: {hit.get('description', '')}"
                for hit in rag_hits
                if hit.get('name') or hit.get('description')
            ]
        except Exception as rag_err:
            logger.warning("[RAG] search failed: %s", rag_err)
            rag_examples = []  # degrade gracefully — RAG bersifat opsional, bukan blocking
    ...
    logger.info(
        "[REVERSE] Sending workflow %s to Ollama",
        workflow_id
    )

    # 7. CALL LLM
    logger.info("STEP 5: Ollama start")
    try:
        ...
        logger.info(
            "[REVERSE] Ollama response length=%d",
        ...
        logger.info("STEP 6: Ollama done")
        return raw_output

    except Exception as exc:
        logger.exception("[REVERSE] Ollama generation failed")
        raise RuntimeError(
            f"Reverse generation failed: {repr(exc)}"
        )